...
@testitem "NFDRS Fuel Models" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    @test model_a.W1 ≈ 0.2 * TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-6  # kg/m²
    @test model_a.DEPTH ≈ 0.8 * FT_TO_M atol = 1.0e-6  # m
    ...
    @test model_a.SCM ≈ 300.0 * FPM_TO_MS atol = 1.0e-6  # m/s
    ...
    @test model_b.WWOOD ≈ 11.5 * TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-5  # Heavy woody load
    ...
    @test model_l.W1 ≈ 0.25 * TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-6
    ...
    @test model_l.WHERB ≈ 0.5 * TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-6
    @test model_l.DEPTH ≈ 1.0 * FT_TO_M atol = 1.0e-6
    @test model_l.MXD == 15.0

    # Test Short needle heavy dead (Model G) - has 1000-hr fuels
    model_g = get_fuel_model(:G)
    @test model_g.W1000 ≈ 12.0 * TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-5  # Heavy 1000-hr load
...
@testitem "EquilibriumMoistureContent Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob2 = ODEProblem(
        emc_compiled,
        Dict(emc_compiled.TEMP => temp_K_low, emc_compiled.RH => 0.3),
        (0.0, 1.0)
    )
    sol2 = solve(prob2)
    ...
    prob3 = ODEProblem(
        emc_compiled,
        Dict(emc_compiled.TEMP => temp_K_low, emc_compiled.RH => 0.8),
        (0.0, 1.0)
    )
    sol3 = solve(prob3)
...
@testitem "OneHourFuelMoisture Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob_rain = ODEProblem(
        fm1_compiled,
        Dict(
            fm1_compiled.EMCPRM => 0.1,
            fm1_compiled.MC10 => 0.1,
    ...
    @test sol_no_sticks[fm1_compiled.MC1][end] ≈ 1.03 * 0.1 atol = 0.001

    # Test 3: With sticks, MC1 = (4.0 * EMCPRM + MC10) / 5.0 (page 3)
    prob_sticks = ODEProblem(
        fm1_compiled,
        Dict(
            fm1_compiled.EMCPRM => 0.1,
            fm1_compiled.MC10 => 0.15,
    ...
    expected_mc1_sticks = (4.0 * 0.1 + 0.15) / 5.0
...
@testitem "TenHourFuelMoisture Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    sol = solve(prob)
    @test sol[fm10_compiled.MC10][end] ≈ 1.28 * 0.1 atol = 0.001
end
...
@testitem "FuelLoadingTransfer Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob_green = ODEProblem(
        flt_compiled,
        Dict(
            flt_compiled.MCHERB => 1.2,
            flt_compiled.W1 => W1_test,
            flt_compiled.WHERB => WHERB_test
        ),
        (0.0, 1.0)
...
@testitem "WoodyFuelMoisture Climate Parameters" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob_frozen = ODEProblem(
        wfm_compiled,
        Dict(
            wfm_compiled.MC1000 => 0.2,
    ...
    @test sol_frozen[wfm_compiled.MCWOOD][end] ≈ 0.6 atol = 0.01
...
@testitem "Fuel Loading Conversion" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    @test WildlandFire.fuel_loading_to_kg_per_sqm(1.0) ≈ TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-6

    # Test deprecated conversion (tons/acre to lb/ft²) still works for backward compatibility
    @test WildlandFire.fuel_loading_to_lb_per_sqft(1.0) ≈ 0.0459137 atol = 1.0e-6

    # Verify: 0.20 tons/acre (Model A) converts correctly
    @test WildlandFire.fuel_loading_to_kg_per_sqm(0.2) ≈ 0.2 * TONS_PER_ACRE_TO_KG_PER_M2 atol = 1.0e-6
    @test WildlandFire.fuel_loading_to_lb_per_sqft(0.2) ≈ 0.00918274 atol = 1.0e-6
end
...
@testitem "HundredHourFuelMoisture Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob = ODEProblem(
        fm100_compiled,
        Dict(
            fm100_compiled.EMCMIN => EMCMIN_test,
...
@testitem "ThousandHourFuelMoisture Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob = ODEProblem(
        fm1000_compiled,
        Dict(
            fm1000_compiled.EMCBAR => EMCBAR_test,
...
@testitem "SpreadComponent Fuel Model A" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob = ODEProblem(
        ...
        Dict(
            ...
            sc_compiled.MC100 => 0.1,
            sc_compiled.MCHERB => 0.3,
            sc_compiled.MCWOOD => 0.6,
            # Other parameters
    ...
    @test sol_wet[sc_compiled.SC][end] ≈ 0.0 atol = 1.0e-10
end
...
@testitem "EnergyReleaseComponent Fuel Model G" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob = ODEProblem(
        ...
        Dict(
            ...
            erc_compiled.MC100 => 0.1,
            erc_compiled.MC1000 => 0.15,
            erc_compiled.MCHERB => 1.2,
            erc_compiled.MCWOOD => 1.0,
            # Other parameters
    ...
    prob_low_fuel = ODEProblem(
        ...
        Dict(
            ...
            erc_compiled.HD => fm.HD,
            erc_compiled.HL => fm.HL,
            erc_compiled.DEPTH => fm.DEPTH * 0.5
        ),  # Reduce depth proportionally
        (0.0, 1.0)
    )
...
@testitem "IgnitionComponent Implementation" setup = [NFDRSSetup] tags = [:nfdrs] begin
    ...
    prob_dry = ODEProblem(
        ic_compiled,
        Dict(
            ic_compiled.TMPPRM => temp_K,
            ic_compiled.MC1 => 0.03,
            ic_compiled.SC => 0.5,  # m/s
