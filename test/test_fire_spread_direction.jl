...
@testitem "FireSpreadDirection - Wind Aligned with Slope" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    @test ustrip(sol[compiled_sys.Y]) ≈ 0.0 atol=1e-10

    # Direction of max spread should be 0 (aligned with slope)
    @test ustrip(sol[compiled_sys.α]) ≈ 0.0 atol=1e-10

    # D_S and D_W calculations
    expected_D_S = R0_val * φs_val * t_val
    expected_D_W = R0_val * φw_val * t_val
    @test ustrip(sol[compiled_sys.D_S]) ≈ expected_D_S rtol=1e-6
    @test ustrip(sol[compiled_sys.D_W]) ≈ expected_D_W rtol=1e-6

    # When aligned, D_H = D_S + D_W
    expected_D_H = expected_D_S + expected_D_W
    @test ustrip(sol[compiled_sys.D_H]) ≈ expected_D_H rtol=1e-6

    # R_H = R0 + D_H/t
    expected_R_H = R0_val + expected_D_H / t_val
    @test ustrip(sol[compiled_sys.R_H]) ≈ expected_R_H rtol=1e-6

    # φ_E = R_H/R0 - 1 = (φw + φs)
    expected_φ_E = φw_val + φs_val
    @test ustrip(sol[compiled_sys.φ_E]) ≈ expected_φ_E rtol=1e-6
end
...
@testitem "FireSpreadDirection - Wind Perpendicular to Slope" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    @test ustrip(sol[compiled_sys.X]) ≈ expected_D_S rtol=1e-6
    @test ustrip(sol[compiled_sys.Y]) ≈ expected_D_W rtol=1e-6
    ...
    @test ustrip(sol[compiled_sys.α]) ≈ expected_α rtol=1e-6
end
...
@testitem "EllipticalFireSpread - Eccentricity Bounds" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    for Z_val in [1.5, 2.0, 3.0, 5.0]
        ...
        @test ustrip(sol[compiled_sys.e]) ≈ expected_e rtol=1e-6
...
@testitem "EllipticalFireSpread - Head Fire Direction" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    @test ustrip(sol[compiled_sys.R_γ]) ≈ R_H_val rtol=1e-6
end
...
@testitem "EllipticalFireSpread - Back Fire Direction" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    @test ustrip(sol[compiled_sys.R_γ]) ≈ ustrip(sol[compiled_sys.R_B]) rtol=1e-6
end
...
@testitem "EllipticalFireSpread - Fire Dimensions" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    # L = D_H + D_B
    @test ustrip(sol[compiled_sys.L]) ≈ ustrip(sol[compiled_sys.D_H]) + ustrip(sol[compiled_sys.D_B]) rtol=1e-6

    # W = L / Z
    @test ustrip(sol[compiled_sys.W]) ≈ ustrip(sol[compiled_sys.L]) / Z_val rtol=1e-6

    # D_F = W / 2
    @test ustrip(sol[compiled_sys.D_F]) ≈ ustrip(sol[compiled_sys.W]) / 2.0 rtol=1e-6

    # Ellipse dimensions
    @test ustrip(sol[compiled_sys.f]) ≈ ustrip(sol[compiled_sys.L]) / 2.0 rtol=1e-6
    @test ustrip(sol[compiled_sys.h]) ≈ ustrip(sol[compiled_sys.D_F]) rtol=1e-6
end
...
@testitem "FirePerimeterSpread - Head Fire Perimeter" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    @test ustrip(sol[compiled_sys.θ]) ≈ 0.0 atol=1e-6
    ...
    @test ustrip(sol[compiled_sys.R_ψ]) ≈ expected_R_ψ rtol=1e-3
...
@testitem "FirePerimeterSpread - Fireline Intensity" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    @test ustrip(sol[compiled_sys.I_B]) ≈ H_A_val * ustrip(sol[compiled_sys.R_ψ]) rtol=1e-6
...
@testitem "FirePerimeterSpread - Backing Fire Perimeter" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    # Head fire case
    prob_head = NonlinearProblem(compiled_sys, Dict(
        compiled_sys.R_H => R_H_val,
        compiled_sys.f => f_val,
        compiled_sys.g => g_val,
        compiled_sys.h => h_val,
    ...
    # Back fire case
    prob_back = NonlinearProblem(compiled_sys, Dict(
        compiled_sys.R_H => R_H_val,
        compiled_sys.f => f_val,
        compiled_sys.g => g_val,
        compiled_sys.h => h_val,
...
@testitem "Integration - Fire Spread Direction to Elliptical Shape" setup=[FireSpreadDirectionSetup] tags=[:fire_spread_direction] begin
    ...
    prob_dir = NonlinearProblem(compiled_dir, Dict(
    ...
    prob_ellipse = NonlinearProblem(compiled_ellipse, Dict(
